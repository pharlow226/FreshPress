 ...
 */
import { BarChart3, Package, Users, DollarSign, UserCircle, ClipboardList, LogOut, Settings } from 'lucide-react';
import {
...
const navItems = [
  ...
  { title: 'Customers',          icon: UserCircle,   key: 'customers' },
  { title: 'Activity Log',       icon: ClipboardList,key: 'activity' },
  { title: 'Company Settings',   icon: Settings,     key: 'settings' },
];
...
export function AdminSidebar({ activePage, onNavigate, onLogout, userName, userRole }: AdminSidebarProps) {
  return (
            ...
            <SidebarMenu>
              {navItems.map((item) => (
                <SidebarMenuItem key={item.key}>
                  <SidebarMenuButton isActive={activePage === item.key} onClick={() => onNavigate(item.key)} tooltip={item.title}>
